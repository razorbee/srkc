...
class DTCoreShortcodesDefination {
	...
	function dt_sc_image_caption($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-image-caption {$type} {$class}'>";
			...
			if(!empty($a_href)):
					$html_1 .= '<a href="'.$a_href.'" alt="'.$a_title.'" title="'.$a_title.'">'; 
				endif;
				if(!empty($image))
			...
			$html_2 = "<div class='dt-sc-image-content'>";
				...
				if(!empty($a_href)):
						$html_2 .= '<a href="'.$a_href.'" alt="'.$a_title.'" title="'.$a_title.'">'; 
				endif;
					$html_2 .= "<h3>{$title}</h3>";
				if(!empty($a_href)):
						$html_2 .= '</a>';
	...
	function dt_sc_tm_carousel_wrapper($attrs, $content = null) {
		...
		$out = "<div class='dt-sc-testimonial-wrapper carousel_items {$type} {$class}' data-animation='{$animation}' data-auto='{$auto}'>";
	...
	function dt_sc_twitter_tweets($attrs, $content = null) {
		...
		if($username && $consumerkey && $consumersecret && $accesstoken && $accesstokensecret) {
			...
			require_once(AUDITING_CORE_PLUGIN."/apis/twitteroauth/twitteroauth.php");
						...
						if( $post_meta['post-gallery-items'] != '' ) :
							...
							$out .= '	<ul class="entry-gallery-post-slider">';
											...
											foreach ( $items as $item ) {
												$out .= '<li>'.wp_get_attachment_image( $item, 'full' ).'</li>';
											}
							$out .= '	</ul>';
							$out .= '	<div class="entry-format '.esc_attr($hide_post_format).'">';
							$out .= '		<a class="ico-format" href="'.esc_url(get_post_format_link( $format )).'"></a>';
					...
					if( $style == "entry-date-left") :
						...
						$out .= '	</div>';
									...
									if( strtolower($allow_excerpt) == 'yes' ):
										$out .= '<div class="entry-body">'.auditing_excerpt($excerpt_length).'</div>';
									endif;
	...
	function dt_sc_down_count($attrs, $content = null) {
		...
		$out .= '<div class="downcount '.$class.'" data-date="'.auditing_cs_get_option('comingsoon-launchdate', date('m/d/Y')).'" data-offset="'.auditing_cs_get_option('comingsoon-timezone', '0').'">';
	...
	function dt_sc_sociable($attrs, $content = null) {
		...
		foreach( $socials as $social ){
		   if( array_key_exists( $social, $social_opts ) ) {
			   $list .= '<li><a  class="fa fa-'.esc_attr( $social ).'" target="_blank" href="'.esc_attr( $social_opts[$social] ).'"></a></li>';
		   }
		}

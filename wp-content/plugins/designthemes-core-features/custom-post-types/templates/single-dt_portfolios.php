                            	...
                            	if( array_key_exists("portfolio-video",$portfolio_settings) && $portfolio_settings['portfolio-video']!='') {
									...
									echo '<li>';
										if (strpos($portfolio_video_url, "vimeo")) :
											...
											echo "<iframe src='http".auditing_ssl()."://player.vimeo.com/video/{$url}' width='1170' height='700' frameborder='0'></iframe>";
										elseif(strpos($portfolio_video_url, "?v=")):
											$url = substr( strrchr($portfolio_video_url, "="),1);
											echo "<iframe src='http".auditing_ssl()."://www.youtube.com/embed/{$url}?wmode=opaque'  width='1170' height='700' frameborder='0'></iframe>";
										endif;
									echo '</li>';
									}
								}
                                    	...
                                    	if( array_key_exists("portfolio-video",$portfolio_settings) && $portfolio_settings["portfolio-video"]!="" ) {
											$portfolio_video_final = array();
												...
												foreach ( $portfolio_video_final as $portfolio_video_url ) {
													if (strpos($portfolio_video_url, "vimeo")) :
														$url = substr( strrchr($portfolio_video_url, "/"),1);
														echo "<a class='video' data-slide-index='".$j."' href=''><img src='".auditing_getVimeoThumb($url)."' alt=".__('Vimeo Video','designthemes-core')." /></a>";													
													elseif(strpos($portfolio_video_url, "?v=")):
														$url = substr( strrchr($portfolio_video_url, "="),1);
														echo "<a class='video' data-slide-index='".$j."' href=''><img src='http".auditing_ssl()."://img.youtube.com/vi/{$url}/0.jpg' alt=".__('Youtube Video','designthemes-core')." /></a>";
													endif;

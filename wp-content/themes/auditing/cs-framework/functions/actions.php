    ...
    if( ! empty( $jsons ) ) {
      ...
      foreach ( $jsons as $path ) {
        ...
        if( is_object( $object ) ) {
          ...
          echo '<h4 class="cs-icon-title">'. esc_html__( 'Error! Can not load json file.', 'auditing' ) .'</h4>';
        }
...
if( ! function_exists( 'cs_set_icons' ) ) {
  function cs_set_icons() {
    ...
    echo '<div id="cs-icon-dialog" class="cs-dialog" title="'. esc_attr__( 'Add Icon', 'auditing' ) .'">';
    echo '<div class="cs-dialog-header cs-text-center"><input type="text" placeholder="'. esc_attr__( 'Search a Icon...', 'auditing' ) .'" class="cs-icon-search" /></div>';
    echo '<div class="cs-dialog-load"><div class="cs-icon-loading">'. esc_html__( 'Loading...', 'auditing' ) .'</div></div>';
    echo '</div>';

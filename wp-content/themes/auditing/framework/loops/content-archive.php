<?php
	...
	if( have_posts() ) :
                        ...
                        <?php if( $format == "image" || empty($format) ) :
                                if( has_post_thumbnail() ) :?>
                                    <div class="entry-thumb">
                                        <a href="<?php the_permalink();?>" title="<?php printf(esc_attr__('Permalink to %s','auditing'),the_title_attribute('echo=0'));?>"><?php the_post_thumbnail("full");?></a>
                        ...
                        <?php if( $post_style == "entry-date-left"): ?>
                                ...
                                <div class="entry-details">
                                	...
                                	<div class="entry-title">
                                    	<?php if( is_sticky(get_the_id()) ) echo '<span class="sticky-post">'.esc_html__('Featured', 'auditing').'</span>'; ?>
                                    	<h4><a href="<?php the_permalink();?>" title="<?php printf( esc_attr__('Permalink to %s','auditing'), the_title_attribute('echo=0'));?>"><?php the_title(); ?></a></h4>
                                    </div>
                                    
                                   <?php if( !empty($allow_excerpt) && !empty($excerpt) ):?>
                                    		<div class="entry-body"><?php echo auditing_excerpt($excerpt);?></div>
                                   <?php endif;?>
                                   ...
                                   <div class="entry-meta-data">
                                   		...
                                   		<p class="author <?php echo esc_attr( $show_author_meta );?>">
                                            ...
                                            <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                                            	title="<?php esc_attr_e('View all posts by ', 'auditing'); echo get_the_author();?>"><?php echo get_the_author();?></a>
                                        </p>
                                   ...
                                   <?php if( !empty($allow_read_more) && !empty($read_more) ):
								   			if( shortcode_exists( 'dt_sc_button' ) && class_exists( 'Vc_Manager' ) ):
												...
												echo '<a href="'.get_permalink().'" title="'.get_the_title().'" class="dt-sc-button small filled">'.esc_html__('Continue Reading', 'auditing').'</a>';
											endif;
                        ...
                        <?php elseif( $post_style == "entry-date-author-left"):?>
                        		...
                        		<div class="entry-date-author">
                                    ...
                                    <div class="entry-author <?php echo esc_attr( $show_author_meta );?>">
                                    	...
                                    	<a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                                        	title="<?php esc_attr_e('View all posts by ', 'auditing'); echo get_the_author();?>"><span><?php echo get_the_author();?></span></a>
                                    </div>
                        ...
                        <?php else: # Default Post Style ?>
                        		<div class="entry-details">
                                    ...
                                    <div class="entry-meta">
                                    	...
                                    	<div class="date <?php echo esc_attr($show_date_meta);?>"><?php esc_html_e('Posted on','auditing'); ?>  <?php echo get_the_date ( get_option('date_format') );?></div>
                                        
                                        <div class="comments <?php echo esc_attr($show_comment_meta);?>"> / <?php
                                            if( ! post_password_required() ) {
                                                comments_popup_link('<i class="pe-icon pe-chat"> </i> 0', '<i class="pe-icon pe-chat"> </i> 1', '<i class="pe-icon pe-chat"> </i> %', '', '<i class="pe-icon pe-chat"> </i>');                                                
                                            } else {
                                                echo '<i class="pe-icon pe-lock"> </i>'. esc_html__('Enter your password to view comments.', 'auditing');
                                            }?>								
                                        </div>
        ...
        <!-- **Pagination** -->
        <div class="pagination blog-pagination"><?php echo auditing_pagination();?></div><!-- **Pagination** -->
        <!-- Blog Template Ends --><?php

	else:?>
    	<h2><?php esc_html_e('Nothing Found.', 'auditing'); ?></h2>
        <p><?php esc_html_e('Apologies, but no results were found for the requested archive.', 'auditing'); ?></p><?php
	endif;?>
 ...
 * Theme support
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_features')) {

	function auditing_features() {
		...
		if ( function_exists( 'add_theme_support' ) ) {
			...
			add_image_size( 'auditing-blog-thumb', 150, 120, true  ); 	// blog - list
			add_image_size( 'auditing-event-list', 420, 336, true  ); 	// event-calendar - list
			add_image_size( 'auditing-event-single-type1', 773, 520, true  ); // event-calendar - single
			add_image_size( 'auditing-event-single-type4', 570, 460, true  ); // event-calendar - single
			add_image_size( 'auditing-event-single-type5', 746, 770, true  ); // event-calendar - single
			add_image_size( 'auditing-event-list2', 420, 275, true  ); 	// event-calendar - shortcode list
		}
		...
		$skin = get_theme_mod('use-predefined-skin',auditing_defaults( 'use-predefined-skin' ) );

		if( $skin ) {
			$skin            = get_theme_mod('predefined-skin',auditing_defaults( 'predefined-skin' ) );
			$colors          = auditing_skins( $skin );
		...
		add_theme_support( 'editor-color-palette', array(
			array(
				'name'  => esc_html__( 'Primary Color', 'auditing' ),
				'slug'  => 'primary',
				'color' => $primary_color,
			),
			array(
				'name'  => esc_html__( 'Secondary Color', 'auditing' ),
				'slug'  => 'secondary',
				'color' => $secondary_color,
			),
			array(
				'name'  => esc_html__( 'Tertiary Color', 'auditing' ),
				'slug'  => 'tertiary',
				'color' => $tertiary_color,
			)
		));				
	}
	add_action('after_setup_theme', 'auditing_features');
}

/* ---------------------------------------------------------------------------
 * Check Activate Theme
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_activation_function')) {
	function auditing_activation_function( $old_name, $old_theme ) {
		...
		$options = array (
		  ...
		  'post-archives-readmore' => '[dt_sc_button style="filled" title="Continue Reading" target="_blank" /]',
		...
		if(!in_array($old_name, array('Auditing', 'Auditing Child'))) {
			update_option( '_cs_options', $options );
		}
	}
	add_action("after_switch_theme", "auditing_activation_function", 10, 2 );
}

/* ---------------------------------------------------------------------------
 *	Under Construction
 * --------------------------------------------------------------------------- */
if( ! function_exists('auditing_under_construction') ){
	function auditing_under_construction(){
...
if( cs_get_option( 'enable-comingsoon' ) ):
	add_action('template_redirect', 'auditing_under_construction', 30);

	// getting shortcode css ----------------------
	add_action('wp_enqueue_scripts', 'auditing_rand_css', 101);
	if (!function_exists('auditing_rand_css')) {
	function auditing_rand_css() {
		$id = cs_get_option( 'comingsoon-pageid' );
		if ( $id ) {
			$shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
			if ( ! empty( $shortcodes_custom_css ) ) {
				
				wp_register_style( 'vc_shortcodes-custom-'.$id, '', false, AUDITING_THEME_VERSION, 'all' );	
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('auditing_wp_list_categories') ){
	function auditing_wp_list_categories( $output ){
	...
	add_filter('wp_list_categories', 'auditing_wp_list_categories');
}

/* ---------------------------------------------------------------------------
 * Filter to modify default list archive widget view
 * --------------------------------------------------------------------------- */
if( !function_exists('auditing_wp_list_archive') ){
	function auditing_wp_list_archive( $link_html,$url, $text, $format, $before, $after ) {
	...
	add_filter('get_archives_link', 'auditing_wp_list_archive', 10, 6);	
}

/* ---------------------------------------------------------------------------
 * Filter to execute shortcode inside contact form7
 * --------------------------------------------------------------------------- */
if( !function_exists('auditing_wpcf7_form_elements') ){
	function auditing_wpcf7_form_elements($form) {
		$form = do_shortcode( $form );
		return $form;
	}
	add_filter('wpcf7_form_elements', 'auditing_wpcf7_form_elements');
}

/* ---------------------------------------------------------------------------
 * Get Current Page or Page ID
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_ID')) {
function auditing_ID(){
	global $post;
	$postID = false;
 ...
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_slider')) {
function auditing_slider( $id = false ){
		...
		if( is_array( $slider_key) && $slider_key['slider_type'] != '' ) {
			...
			if( ( $slider_key['slider_type'] == 'revolutionslider' ) && ( $slider_key['revolutionslider_id'] != '' ) && ( $slider_key['show_slider'] == 'true' ) ){
				...
				$slider = '<div id="slider"><div class="dt-sc-main-slider" id="dt-sc-custom-slider">';
					$slider .= auditing_wp_kses(do_shortcode( $slider_key['customslider_sc'] ));
				$slider .= '</div></div>';
			}
		}
	}
 ...
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_pagination')) {
function auditing_pagination( $query = false, $load_more = false ){
	global $wp_query;
		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
						...
						$output .= '<span class="button_label">'. esc_html__('Load more', 'auditing') .'</span>';
					$output .= '</a>';
				$output .= '</div>';
			}
			...
			$output .= '<div class="column one pager_wrapper">';
				...
				$args = array(
					...
					'mid_size'           => $mid_size,
					'prev_next'          => true,
					'prev_text'          => '<i class="fa fa-angle-double-left"></i>'.esc_html__('Prev', 'auditing'),
					'next_text'          => esc_html__('Next', 'auditing').'<i class="fa fa-angle-double-right"></i>',
					'type'               => 'list'
				);
 ...
 * Page Title
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'auditing_page_title' ) ){
	function auditing_page_title( $echo = false ){
		...
		if( function_exists('tribe_is_month') && ( tribe_is_event_query() || tribe_is_month() || tribe_is_event() || tribe_is_day() || tribe_is_venue() ) ){
			...
			$title = get_the_title( auditing_ID() );
 ...
 * Breadcrumbs
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_breadcrumbs')) {
function auditing_breadcrumbs(){
	global $post;
	
	$homeLink = esc_url( home_url('/') );
	$separator = '<span class="'.get_theme_mod( 'breadcrumb-delimiter', auditing_defaults('breadcrumb-delimiter') ).'"></span>';
	...
	if( function_exists('is_bbpress') && is_bbpress() ){
		bbp_breadcrumb( array(
			...
			'after' 		=> '</div>',
			'sep' 			=> '<span class="'.get_theme_mod( 'breadcrumb-delimiter', auditing_defaults('breadcrumb-delimiter') ).'"></span>',
			'crumb_before' 	=> '',
			'crumb_after' 	=> '',
			'home_text' 	=> esc_html__('Home','auditing'),
		) );
		return true;
	}
	...
	$breadcrumbs[] =  '<a href="'. $homeLink .'">'. esc_html__('Home','auditing') .'</a>';
	...
	if( function_exists('tribe_is_month') && ( tribe_is_event_query() || tribe_is_month() || tribe_is_event() || tribe_is_day() || tribe_is_venue() ) ) {
		...
		$breadcrumbs[] = '<a href="">'.esc_html__('Search', 'auditing').'</a>';

	} elseif( is_post_type_archive('dt_portfolios') ){

		$breadcrumbs[] = '<span class="current">'. esc_html__('Portfolios', 'auditing') .'</span>';
	} elseif( is_post_type_archive('product') ){

		$breadcrumbs[] = '<span class="current">'. esc_html__('Shop', 'auditing') .'</span>';
	} 	elseif( is_single() && ! is_attachment() ){

		if( get_post_type() == 'dt_portfolios' ){

			$cat = get_the_term_list(auditing_ID(), 'portfolio_entries', '', '$$$', '');
			...
			$cat = get_the_term_list(auditing_ID(), 'product_cat', '', '$$$', '');
			$cats = array_filter(explode('$$$', $cat));
			if (!empty($cats))
				$breadcrumbs[] = $cats[0];
		...
		$breadcrumbs[] = '<span class="current">'. get_the_title( auditing_ID() ) .'</span>';
 ...
 * Breadcrumb Section
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_breadcrumb_section')) {
function auditing_breadcrumb_section($id, $post_type ) {
	
	$bcrumb = (int) get_theme_mod( 'show-breadcrumb', auditing_defaults('show-breadcrumb') );
	if( !empty( $bcrumb ) ) :
		...
		if( array_key_exists('breadcrumb_background', $settings) ):
			...
			if(!empty($bg) || !empty($co)) :
				...
				$pos = get_theme_mod( 'breadcrumb-bg-position', auditing_defaults('breadcrumb-bg-position') );
				$pos = !empty($position) ? $position : 'center center';
				$repeat = get_theme_mod( 'breadcrumb-bg-repeat', auditing_defaults('breadcrumb-bg-repeat') );
		...
		$bstyle = get_theme_mod( 'breadcrumb-style', auditing_defaults('breadcrumb-style') );
...
if (!function_exists('auditing_breadcrumb_section_with_class')) {
function auditing_breadcrumb_section_with_class ( $title , $class ){
...
if (!function_exists('auditing_events_title')) {
function auditing_events_title() {
	...
	if( tribe_is_month() && !is_tax() ) { 
		$title = sprintf( esc_html__( 'Events for %s', 'auditing' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() )  {
		$title = sprintf( esc_html__('Events for week of %s', 'auditing'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
		$title = esc_html__( 'Events for', 'auditing' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
		if( tribe_is_past() ) {
			$title = esc_html__( 'Past Events', 'auditing' );
		} else {
			$title = esc_html__( 'Upcoming Events', 'auditing' );
		}
		...
		$title = esc_html__('Tag Archives','auditing');
	}
	return $title;
}
}
 ...
 * Excerpt
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_excerpt')) {
function auditing_excerpt($limit = NULL) {
 ...
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_wp_kses')) {
function auditing_wp_kses($content) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('auditing_hex2rgb')) {

	function auditing_hex2rgb($hex) {
 ...
 * Theme Header Logo
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_header_logo')) {
function auditing_header_logo() {
	echo '<div id="logo">';
		if( class_exists( 'Kirki' ) ) {
			$use_logo   = (int) get_theme_mod( 'use-custom-logo', auditing_defaults('use-custom-logo') );
			$url        = get_theme_mod( 'custom-logo', auditing_defaults('custom-logo') );
			$darkbg_url = get_theme_mod( 'custom-dark-logo', auditing_defaults('custom-dark-logo') );
			...
			if( !empty( $use_logo ) && !empty( $url ) ) {?>
				<a href="<?php echo esc_url( home_url('/') ); ?>" rel="home">
					...
					<img class="darkbg_logo" src="<?php echo esc_url( $darkbg_url ); ?>" alt="<?php echo esc_attr( get_bloginfo( 'name') ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name') ); ?>"/>
				</a><?php
			}

			if( empty( $use_logo ) ){?>
				<div class="logo-title">
					<h1 id="site-title"><a href="<?php echo esc_url(home_url('/')); ?>" title="<?php echo esc_attr( get_bloginfo( 'name') ); ?>"><?php bloginfo('title'); ?></a></h1>
				</div><?php
			}
		} else {?>
			<a href="<?php echo esc_url(home_url('/'));?>" title="<?php echo esc_attr( get_bloginfo( 'name') ); ?>">
				<img src="<?php echo AUDITING_THEME_URI.'/images/logo.png'; ?>" alt="<?php echo esc_attr( get_bloginfo( 'name') ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name') ); ?>"/>
			</a><?php
		}
	echo '</div>';
}
 ...
 * Theme Comment Style
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_comment_style')) {
function auditing_comment_style( $comment, $args, $depth ) {
	...
	case 'trackback':
		...
		echo "<p>";
		esc_html_e('Pingback:', 'auditing');
		comment_author_link();
		edit_comment_link(esc_html__('Edit', 'auditing'), ' ', '');
		echo "</p>";
		break;
	...
	case '':
		...
		echo '  <div class="comment-body">';
					...
					if ($comment->comment_approved == '0') :
						esc_html_e('Your comment is awaiting moderation.', 'auditing');
					endif;
					edit_comment_link(esc_html__('Edit', 'auditing'));
		echo '	</div>';
		echo '	<div class="reply">';
		echo 		comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'auditing'), 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '	</div>';
		echo '</section>';
 ...
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_get_page_permalink_by_its_template')) {
function auditing_get_page_permalink_by_its_template( $template ) {
	$permalink = '#';
 ...
 * Theme show sidebar
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_show_sidebar')) {
function auditing_show_sidebar( $type, $id, $position = 'right' ) {
 ...
 * --------------------------------------------------------------------------- */
if (!function_exists('auditing_global_variables')) {
function auditing_global_variables($variable = '') {

	global $woocommerce, $product, $woocommerce_loop, $post, $wp_query, $pagenow, $auditing_like_love;
	...
	switch($variable) {
		...
		case 'pagenow':
			return $pagenow;
		break;
		case 'auditing_like_love':
			return $auditing_like_love;
		break;
	}
	return false;
}

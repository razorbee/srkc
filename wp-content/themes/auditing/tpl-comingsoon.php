...
<div class="wrapper under-construction <?php echo esc_attr($type); ?>" style="<?php echo esc_attr($style); ?>"><?php
	...
	if( !empty($pageid) ):
		...
		echo auditing_wp_kses( DTCoreShortcodesDefination::dtShortcodeHelper( stripslashes( $page['post_content'] ) ) );
	else:
		echo '<div class="uc-wrapper-inner">';
			echo '<h2>'.esc_html__('Website is almost ready', 'auditing').'</h2>';
			echo '<p>'.esc_html__('Our website is under construction.', 'auditing').'</p>';
			echo '<p>'.esc_html__("We'll be here soon with our new awesome.", 'auditing').'</p>';
			...
			if( cs_get_option( 'show-launchdate' ) == 'true' ):
				...
				echo '<div class="downcount" data-date="'.$date.'" data-offset="'.$offset.'">';
					echo '<div class="dt-sc-counter-wrapper">';
						...
						echo '</div>';
						echo '<h3 class="title">'.esc_html__('Days', 'auditing').'</h3>';
					echo '</div>';
					echo '<div class="dt-sc-counter-wrapper">';
						echo '<div class="counter-icon-wrapper">';
							echo '<div class="dt-sc-counter-number hours">00</div>';
						echo '</div>';
						echo '<h3 class="title">'.esc_html__('Hours', 'auditing').'</h3>';
					echo '</div>';
					echo '<div class="dt-sc-counter-wrapper">';
						echo '<div class="counter-icon-wrapper">';
							echo '<div class="dt-sc-counter-number minutes">00</div>';
						echo '</div>';
						echo '<h3 class="title">'.esc_html__('Minutes', 'auditing').'</h3>';
					echo '</div>';
					echo '<div class="dt-sc-counter-wrapper last">';
						echo '<div class="counter-icon-wrapper">';
							echo '<div class="dt-sc-counter-number seconds">00</div>';
						echo '</div>';
						echo '<h3 class="title">'.esc_html__('Seconds', 'auditing').'</h3>';
					echo '</div>';
				echo '</div>';
			endif;
		echo '</div>';
